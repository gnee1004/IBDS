...
# af.md #8 수정 — _resolve_case_revisit_url: 재방문 목적지 허용 범위(원본과 동일 host) 검사
class ResolveCaseRevisitUrlTests(unittest.TestCase):
    def test_location_to_external_host_is_rejected_by_scope_check(self):
        ...
        self.assertIsNone(resolved)  # 호출부가 family.revisit_url로 폴백

    def test_location_to_same_host_is_accepted(self):
        case = SimpleNamespace(url="http://internal.example.com/submit")
        sent = {"response_headers": {"location": "http://internal.example.com/thanks"}}

        resolved = orchestrator._resolve_case_revisit_url(sent, case)

        self.assertEqual(resolved, "http://internal.example.com/thanks")
